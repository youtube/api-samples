...
*   All library required are on "requirements.txt", just type "pip install -r requirements.txt"
...
### Changes:
    The default httplib isn't maintained anymore, so we use httplib2, it means that some
    errors/imports from httplib do not exists on httplib2 so I removed all errors/imports
    that does not exist anymore on httplib2

    I've done my best to make the indentation 4 spaces (1 tab), some files could be messy, but it is
    very easy to fix

    I've only tested the "upload_video.py" and after an indentation fix, it worked like a charm, I guess that all the other scripts will work fine too (indentation fix probably will be needed)
