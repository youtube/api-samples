		...
		for language in update_result['localizations']:
			...
			if language.lower() == args.language.lower():
				...
				print(f"""Updated playlist 
           				\'{args.playlist_id}\'
               			localized title to \'{localization["title"]}\'
                  		and description to \'{localization["description"]}\'
...
def get_playlist_localization(youtube, args):
	...
	print(f'Playlist title is "{localized["title"]}" and description is "{localized["description"]}" in language "{args.language}"')
